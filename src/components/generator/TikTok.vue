<template>
  <div class="tiktok">
    <v-tooltip text="Edit the text to create your own TikTok-style logo" location="top" model-value>
    ...
    <div class="customize mt-3">
      <v-tooltip text="Customize your TikTok-style logo" location="top" model-value>
        <template v-slot:activator="{ props }">
          <div v-bind="props" class="customize-color">
            ...
            <div class="flex items-center">
              Glitch Effect: <v-checkbox-btn v-model="glitchEffect"></v-checkbox-btn>
            </div>
          </div>
        </template>
      </v-tooltip>
...
const twitter = () => {
  ...
  let text = encodeURIComponent(`Created my TikTok-style logo with Porn-Logo.com! ${url}`);
...
.box {
  ...
  .editarea {
    ...
    .logo-text {
      ...
      font-weight: 900;
      position: relative;
      text-shadow: 3px 3px 0 #FE2C55, -3px -3px 0 #00F2EA;
    }
  }
}
